...
ON b.facid = f.facid
WHERE f.name LIKE 'TENNI%'
ORDER BY MemName, Facility;
...
SELECT m.firstname|| ' ' || m.surname AS MemName, 
f.name AS Facility, 
f.facid AS FacID, 
b.slots AS Num, 
f.membercost AS MemCostCheck, 
f.guestcost AS GuestCostCheck,
...
SELECT (SELECT (firstname|| ' ' || surname) FROM Members) AS MemName , 
f.name AS Facility, 
f.facid AS FacID, 
b.slots AS Num, 
f.membercost AS MemCostCheck, 
...
SELECT ( m.surname || ', ' || m.firstname ) AS MemName, ( c.surname || ', ' || c.firstname ) AS Recommender
FROM Members AS m
